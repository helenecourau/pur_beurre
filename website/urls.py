...
urlpatterns = [
    ...
    url('accueil',  TemplateView.as_view(template_name='website/home.html'), name="home"),
    url('mentions-legales',  TemplateView.as_view(template_name='website/legal.html')),
    ...
    path('produit/<int:id>-<slug:slug>', views.product_page, name='product_page'),
    url(r'^resultats/(?P<page>\d+)$', views.result, name='result'),
    url(r'^recherche/(?P<page>\d+)$', views.search, name='search'),
    url(r'^mes-aliments/(?P<page>\d+)$', views.my_products, name='my_products'),
]