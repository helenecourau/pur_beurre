...
def result(request):
    ...
    if not products:
        messages.add_message(request, messages.WARNING,
                             "Le produit demandé n'est pas dans notre base de données.\
                              Veuillez recommencer votre recherche.")
    else:
        choice = products[0]
        dico = choice.categories.all()
        substitutes_list =\
            Product.objects.filter(categories__name_cat__exact=dico[0],
                                   grade__in='ab').order_by('name_prod')
        if not substitutes_list:
            messages.add_message(request, messages.WARNING,
                                 "Nous n'avons pas de produit de\
                                  meilleure qualité à vous proposer.")
        else:
...
def my_products(request, page=1):
    if not request.user.is_authenticated:
        ...
        user_id = request.user.id
        my_products_list =\
            Product.objects.filter(fav=user_id).order_by('name_prod')
...
def search(request, page=1):
    query = request.GET.get('query')
    products_list =\
        Product.objects.filter(name_prod__icontains=query).\
        order_by('name_prod')
